...
PixX = 2;                                    % Pixel length in X-direction           [m]
PixY = 2;                                    % Pixel length in Y-direction           [m]
NPixX=100;                                      % Number of Pixels in X-direction        [-]
NPixY=30;                                       % Number of Pixels in Y-direction       [-]
...
dt = 0.1;                                       % calculation time step                         [day] 

% system constants
DemClay = -1;                                   % Dem of Clay above reference level              [m]
K = 0.5;                                        % hydraulic Conductivity                         [m/day]
PorVol = 0.35;                                  % Pore Volume                                    [-]
...
% a = 'DEMPolder.txt';
% DEMPolder1 = fopen(a);
% DEMPolder = fscanf(DEMPolder1,'%f');
% fclose(DEMPolder1);

DEMPolder = importdata('DEMPolder.txt');
...
TotRunoff = TotRunoff + sum(sum(Runoff));           
Time = Time + dt;
...
%%%%%%%%%%%% END DYNAMIC LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot(2,1,1); contourf(x,y,Runoff,20); title('Aerial View'); xlabel('distance [m]'); ylabel('distance [m]');
subplot(2,1,2); plot(x,H(3,:)); title('Cross section'); xlabel('distance [m]'); ylabel('Groundwater level [m]');

TotPrec = 0.01 * PixX * PixY * (NPixX-2) * (NPixY) * (EndTime-StartTime);
Storage = (sum(sum(H-InitH))) * PixX*PixY*PorVol;
